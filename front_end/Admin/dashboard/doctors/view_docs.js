...
window.addEventListener('load',async(e)=>{
    ...
    try {
        ...
        // get doctors
        const doctors = await fetch('/admin/dashboard/doctors',{
            method:'GET'
        })
        ...
        for (let i of doctors_data.doctors){
            
            let pj = document.createElement("div");

            pj.className = `doc-${i.doc_id}`;
            pj.id = `doc-${i.doc_id}`;
            ...
            let hh = document.createTextNode(`Doctors ${i.doc_id}`);
            pj.appendChild(h);
            h.appendChild(hh);
            ...
            editeBtn.addEventListener('click',async () => {
                location.href = `http://test.host:3000/admin/dashboard/edite_doc/${i.doc_id}`
            })


            let h3Message = document.createElement('h3')
            h3Message.id = `h3Message-${i.doc_id}`
            ...
            delBtn.addEventListener('click',async () => {
                ...
                const htmlMessage = document.getElementById(`h3Message-${i.doc_id}`)
                setTimeout(()=>{
                    h3Message.innerText = '';
                },3000)
            })
