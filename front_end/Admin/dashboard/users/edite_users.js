...
window.addEventListener('load',async(e)=>{
    ...
    const user = await fetch(`/admin/dashboard/get_user/${location.href.substring(location.href.lastIndexOf('/') + 1)}`,{
        method:'GET'
    })
    ...
    form.full_name.setAttribute('value',user_data.user.full_name)
    form.username.setAttribute('value',user_data.user.username)
    form.email.setAttribute('value',user_data.user.email)
    form.level.setAttribute('value',user_data.user.level_id)
    form.role.setAttribute('value',user_data.user.role)
...
form.addEventListener('submit', async(e)=>{
    ...
    const user_id = parseInt(location.href.substring(location.href.lastIndexOf('/') + 1));
