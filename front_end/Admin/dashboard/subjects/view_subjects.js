...
let h33 = document.createTextNode(` subjects :-`);
h3.appendChild(h33);
d.appendChild(h3);
...
window.addEventListener('load',async(e)=>{
    ...
    try {
        ...
        // get users
        const subjects = await fetch('/admin/dashboard/subject',{
            method:'GET'
        })
        const subjects_data = await subjects.json()
        
        
        for (let i of subjects_data.subjects){
            
            let pj = document.createElement("div");

            pj.className = `sub-${i.user_id}`;
            pj.id = `sub-${i.user_id}`;
            ...
            let hh = document.createTextNode(`subject ${i.sub_id}`);
            pj.appendChild(h);
            h.appendChild(hh);
            ...
            editeBtn.addEventListener('click',async () => {
                location.href = `http://test.host:3000/admin/dashboard/edite_subject/${i.sub_id}`
            })


            let h3Message = document.createElement('h3')
            h3Message.id = `h3Message-${i.sub_id}`
            ...
            delBtn.addEventListener('click',async () => {
                ...
                const htmlMessage = document.getElementById(`h3Message-${i.sub_id}`)
                setTimeout(()=>{
                    h3Message.innerText = '';
                },3000)
            })
